...
export default function Smartlist() {
  ...
  const { data, isLoading } = useReadContract({
    contract,
    method: resolveMethod("listLicenses"),
    params: []
  });
  ...
  return (
    <Paper sx={{ width: "85%", overflow: "hidden", padding: "12px" }} className="absolute top-[0px] left-[210px]">
      <Divider />
      <Box height={0} />
          ...
          <TableBody>
            {rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((item, index) => (
              <TableRow
                ...
                onClick={() => router.push(`/RTO/rhashl?licenseHash=${hashes[index]}`)}
              >
